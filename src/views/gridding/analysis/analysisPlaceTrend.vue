      ...
      <a-spin :spinning="spinning" size="large">
        <a-row :gutter="24">
          <a-col :md="24" :lg="18">
            ...
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <el-checkbox v-model="pjx_checked" @change="DataValueAvelineOnclick">平均线</el-checkbox>
            <br />
            <!--//echart图表输出在div中-->
...
export default {
  ...
  methods: {
    ...
    init_echart (list_data) {
      ...
      //data_num 为1,2,3 常数
      //遍历给平均数数组附初始值.,避免NaN
      var data_value_aug = new Array();
      var time_data_length = '1';
      var list_data_length = '1';
      var value_aug = '0';
      for (var data_num in list_data) {
        ...
        for (var data_num_small in list_data[data_num]["time_data"]) {
          ...
          if (data_num == 0) {
            data_value_aug.push(list_data[data_num]["time_data"][data_num_small]["type_value"]);
          } else {
            data_value_aug[data_num_small] = data_value[data_num_small] + data_value_aug[data_num_small];
          }
          time_data_length = data_num_small;
        }
        list_data_length = data_num;
      ...
      for (var i = 0; i <= time_data_length; i++) {
        data_value_aug[i] = (parseFloat(data_value_aug[i]) / (parseFloat(list_data_length) + 1)).toFixed(2);
      }
      if (this.pjx_checked == true) {
        option.series.push({
          name: '平均水平线',
          type: 'line',
          data: data_value_aug
